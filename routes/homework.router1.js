  ...
  try {
    ...
    if (req.cookies.student) {
      let student = jwt.verify(req.cookies.student, process.env.SecretPassword);
       stulevel = await user.findOne({ _id: student.studentCard });
      data = await model.find({
        type: "homework",
        educetionlevel:stulevel.educetionlevel, grade:stulevel.grade
      }); 
    ...
    unitcheck = await unit.getModel().findOne({educetionlevel:stulevel.educetionlevel, grade:stulevel.grade});
      checkah = await degreeModel.find({ student: student.studentCard });
      res.render("openHomework.ejs", { data: data, deg: deg, checkah: checkah ,unit:unitcheck,stulevel }); 
    } else {
      res.redirect("/");
    }
  } catch (err) {
